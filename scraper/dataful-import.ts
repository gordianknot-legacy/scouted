/**
 * Dataful CSV Import Script
 *
 * Imports CSR spending data from the Dataful master CSV into both
 ...
 *
 * Usage:
 *   npx tsx dataful-import.ts --file <path-to-csv>
 *   npx tsx dataful-import.ts --file <path-to-csv> --fy 2023-24 --dry-run
 */
...
function getFilePath(): string {
  ...
  console.error('Usage: npx tsx dataful-import.ts --file <path-to-csv>')
  process.exit(1)
}
...
function parseCSV(text: string): DatafulRow[] {
  const lines = text.split('\n')
  if (lines.length < 2) return []

  const headers = lines[0].split(',').map(h => h.trim())
  const results: DatafulRow[] = []
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    results.push(row as unknown as DatafulRow)
  }
...
async function main() {
  ...
  console.log(`\n=== Dataful CSV → ScoutEd Import (FY ${fiscalYear}) ===`)
